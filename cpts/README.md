# HackTheBox - Certified Penetration Tester (CPTS) Path

Solutions, notes, and answers for the HackTheBox Certified Penetration Tester (CPTS) learning path.

---

## Modules

### Phase 1: Core Fundamentals & Reconnaissance
- [01. Penetration Testing Process](./01-penetration-testing-process/README.md)
- [02. Getting Started](./02-getting-started/README.md)
- [03. Network Attacks](./03-network-attacks/README.md)
- 04. Footprinting
- 05. Information Gathering - Web Edition
- 06. Vulnerability Assessment

### Phase 2: Exploitation & Tooling
- 07. File Transfers
- 08. Shells & Payloads
- 09. Using the Metasploit Framework
- 10. Password Attacks
- 11. Attacking Web Applications with Ffuf
- 12. Login Brute Forcing

### Phase 3: Web Application Attacks
- 13. SQL Injection Fundamentals
- 14. SQLMap Essentials
- 15. Cross-Site Scripting (XSS)
- 16. File Inclusion
- 17. Command Injections
- 18. Web Attacks
- 19. Attacking Web Applications

### Phase 4: Enterprise & Post-Exploitation
- 20. Active Directory Enumeration & Attacks
- 21. Linux Privilege Escalation
- 22. Windows Privilege Escalation
- 23. Documentation & Reporting
