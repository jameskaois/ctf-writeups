...
- **Author:** `jameskaois`
- **CTF:** DreamHack
- **Challenge:** PATCH-1 (web)
- **Link**: `https://dreamhack.io/wargame/challenges/70`
- **Level:** `4`
- **Date:** `28-11-2025`
...
Patch all the vulnerabilities and get the flag.

## My Solution

The initial `app.py` has lots of vulnerabilities we can test submitting it to see how many are there.

Vulnerabilities are related to:

- SQL Injeciton in `login()` and `memoView()`.
- SSTI in `memoView()`.
- IDOR in `memoUpdate()`
- Hard-coded key of `app.secret_key`.

Fixed these and get the flag. Fixed full `app.py`:
...
def memoUpdate(idx):
  ...
  # 1. Fetch the memo
  # Note: Use query_db safely. The original code had [0] which crashes if result is empty.
  ...
  if title and contents:
    ...
    # --- PATCH IS HERE ---
    # We add "AND userid=?" to ensure the logged-in user owns this specific memo.
    cur.execute("UPDATE memo SET title=?, contents=? WHERE idx=? AND userid=?",
                (title, contents, idx, userid))
    conn.commit()

    # Check rowcount. If it is 0, it means the ID existed but the UserID didn't match (or memo didn't exist).
    if cur.rowcount > 0:
      return jsonify(result="success")
